...
for file in dane/icao/*; do
    if [ -f "$file" ]; then
        inode=$(stat -c %i "$file")                 # pobiera index inode ze statystyk
        inodes="$inodes"$'\n'"$inode"
    fi
done

echo "$inodes" | sort -u | grep -v '^$' | wc -l     # sort unique, grep do wyszukiwania -bez_wzorca regex, world count lines
