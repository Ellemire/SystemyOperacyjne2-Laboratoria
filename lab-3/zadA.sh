...
find linux-5.11.13 \( -name "core.c" -or -name "irq.c" \)    \
    ! -path "*/mips/*"                                      \
    \(                                                      \
        \( -path "*/bluetooth/*" -and -size +30k \)           \
        -or                                                  \
        \( ! -path "*/bluetooth/*" -and -size -1024c \)       \
    \)


# find linux-5.11.13 ( core.c OR irq.c )
# AND NOT (path contains mips)
# AND (
#       ( NOT in bluetooth AND size ≤ 1KiB )
#       OR
#       ( IN bluetooth AND size ≥ 30KiB )
#     )